...
from app.database import Handshake, Log
...
async def process_message(message: Message):
	...
	log_id = ObjectId()
	Log.insert_one({"id": log_id, "body": body, "processed": False, "created_at": datetime.now(),  "queue": "handshake.queue"})
	...
	Handshake.insert_one(body_data)
...
async def consume_messages():
	...
	queue = await channel.declare_queue("handshake.queue", durable=True)
	
	await queue.bind(exchange, routing_key="handshake.route")
