...
from app.database import Notices, Profiles, Facility, FacilityMetrics, get_db
...
async def start_periodic_task(background_tasks: BackgroundTasks):
    ...
    for item in manifest:
        if item and item.get("totalExpected") == item.get("totalReceived") == item.get("totalQueued"):
            dwapi_version = None
            if item['mfl_code'] in dwapi_versions:
                dwapi_version = dwapi_versions[item['mfl_code']]
            message = str({"Facility": item['facility'], "MFL Code": item['mfl_code'], "Docket": item['docket'],
                           ...
                           "dwapi_version": dwapi_version, "log_date": datetime.datetime.now().isoformat()})
...
async def pull_dwh_indicators(background_tasks: BackgroundTasks, db: Session = Depends(get_db)):
    ...
    for facility in facilities:
        ...
        background_tasks.add_task(
            ...
            db,
            facility,
            f"SELECT SUM(Tested) as value FROM Reporting.dbo.AggregateHTSUptake WHERE MFLCode = {facility['mfl_code']} and year = {year} and month = {month}",
            'HTS_TESTED'
        )
        background_tasks.add_task(
            fetch_and_post,
            db,
            facility,
            f"SELECT SUM(Positive) as value FROM Reporting.dbo.AggregateHTSUptake WHERE MFLCode = {facility['mfl_code']} and year = {year} and month = {month}",
            'HTS_TESTED_POS'
        )
        background_tasks.add_task(
            fetch_and_post,
            db,
            facility,
            f"SELECT SUM(patients_startedART) as value FROM Reporting.dbo.AggregateTxNew WHERE MFLCode = {facility['mfl_code']} and StartARTYearMonth = '{year}-{month}'",
            'TX_NEW'
        )
        background_tasks.add_task(
            fetch_and_post,
            db,
            facility,
            f"SELECT SUM(CountClientsTXCur) as value FROM Reporting.dbo.AggregateTXCurr WHERE MFLCode = {facility['mfl_code']}",
            'TX_CURR'
        )
